...
class bidirectional_Search:
	...
	def bidirectional_search(self, source, last_node):
		...
		self.source_queue.append(source)    # 시작 노드 추가
		self.source_visited[source] = True  # 시작 노드 방문 여부 True
		self.source_parent[source] = -1		# 뭐지이건?
		
		self.last_node_queue.append(last_node)		# 종점 노드 추가
		self.last_node_visited[last_node] = True	# 종점 노드 방문 여부 True
		self.last_node_parent[last_node] = -1		# 뭐지이건?
